...
class ExamsController extends Controller
{
    ...
    {
        if (isset($_POST['subject_id']) && isset($_POST['mark']) && isset($_POST['student_id']) && isset($_POST['course_year_id']) && isset($_POST['semester_id']))
        {
            ...
            if (!$rowId = $studentSubjectMark->isRowAlreadyExist())
            {
                ...
                {
                    $studentSubjectMark->percentage = ($studentSubjectMark->mark / $subject->total_mark) * 100;
                    $studentSubjectMark->grade = $studentSubjectMark->getGrade($studentSubjectMark->percentage);
                }
                if ($studentSubjectMark->save())
                    echo "Mark Added!";
                exit;
            }
            else
            {
                if (!$studentSubjectMark->isAlreadyExist())
                {
                    ...
                    if ($markObj->mark != $_POST['mark'])
                    {
                        $markObj->mark = $_POST['mark'];
                        if ($subject->total_mark)
                        {
                            $markObj->percentage = ($markObj->mark / $subject->total_mark) * 100;
